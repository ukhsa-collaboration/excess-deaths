...
```{r eng-weekly-sans-baseline, fig.height=10, fig.cap="Weekly excess deaths by date of registration, England."}
...
```{r regions-weekly, fig.height=10, results='asis', fig.cap=reg_fig_cap}
...
The trend in excess deaths by region is shown in Figures \@ref(fig:regions-weekly-1) to \@ref(fig:regions-weekly-9), which allows the extent of the excess each week to be compared over time and for selected regions.
...
```{r deaths-by-cause-weekly, fig.height=10, results='asis', fig.cap=ucod_fig_cap}
...
```{r deaths-by-cause-mentions-weekly, fig.height=10, results='asis', fig.cap=mentions_fig_cap}
cat("### Dementia and Alzheimer’s disease \n")
...
Figures \@ref(fig:deaths-by-cause-mentions-weekly-1) to \@ref(fig:deaths-by-cause-mentions-weekly-3) show weekly excess deaths where a specific cause was mentioned anywhere on the death certificate. These causes will have contributed to the death but not necessarily been the underlying cause.
...
```{r place-of-death-weekly, fig.height=10, results='asis', fig.cap=pod_fig_cap}
cat("### Own home \n")
...
Figures \@ref(fig:place-of-death-weekly-1) to \@ref(fig:place-of-death-weekly-5) show the weekly total excess registered deaths by place of death since week ending 27 March 2020. These charts can be used to understand the trend in excess deaths by place of death. This analysis should be interpreted as excess deaths within each place of death compared to what would have been expected based on data over the past five years.
...
server_files <- server_files[grepl("^excess-mortality-in-england", server_files)]
